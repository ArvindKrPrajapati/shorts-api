    ...
    try {
        ...
        if (mobile.length === 10 && otp.length === 6) {
            ...
            if (varify) {
                ...
                var token;
                if (userExists) {
                    token = jwt.sign(JSON.stringify({_id:userExists._id,image:userExists.image, name: userExists.name,isVarified:userExists.isVarified }), process.env.JWT_SECRET);
                } else {
                    const name = "user" + Math.floor(10000 + Math.random() * 90000);
                    const newUser = await user.create({ mobile, name })
                    token = jwt.sign(JSON.stringify({_id:newUser._id,image:newUser.image, name: newUser.name,isVarified:newUser.isVarified }), process.env.JWT_SECRET);
                }
